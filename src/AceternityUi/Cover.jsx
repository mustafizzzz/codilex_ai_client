...
export const Cover = ({ children, className }) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className="relative hover:bg-neutral-900 group/cover inline-block dark:bg-neutral-900 bg-neutral-100 px-2 py-2 transition duration-200 "
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
      ...
      <motion.span
        ...
        transition={{
          duration: 0.2,
          x: { duration: 0.2, repeat: Number.POSITIVE_INFINITY, repeatType: "loop" },
          y: { duration: 0.2, repeat: Number.POSITIVE_INFINITY, repeatType: "loop" },
          scale: { duration: 0.2 },
          filter: { duration: 0.2 },
        }}
...
export const Beam = ({ className, delay, duration, hovered, width = 600, ...svgProps }) => {
  ...
  return (
    <motion.svg width={width ?? "600"} height="1" viewBox={`0 0 ${width ?? "600"} 1`} fill="none" xmlns="http://www.w3.org/2000/svg" className={cn("absolute inset-x-0 w-full", className)} {...svgProps}>
      <motion.path d={`M0 0.5H${width ?? "600"}`} stroke={`url(#${id})`} />
      <defs>
...
export const CircleIcon = ({ className, delay }) => {
  return (
    <div className={cn(`pointer-events-none animate-pulse group-hover/cover:hidden group-hover/cover:opacity-100 group h-2 w-2 rounded-full bg-teal-400 dark:bg-white opacity-60 group-hover/cover:bg-white`, className)}></div>
  );
};
